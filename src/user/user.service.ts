...
export class UserService {
  ...
  async createUserForCRM(dto:CreateCRMUserDto) : Promise<typeHttpResponse<UserResponseInterface>>{
    ...
    const findUserType = await this.typeUserRepository.findOneBy({id:dto.type})
  ...
  async update(id: number, dto: UpdateUserDto): Promise<typeHttpResponse<UserResponseInterface>> {
  ...
  async setAvatar(id: number,file:Express.Multer.File) : Promise<typeHttpResponse<Boolean>>{
    const user = await this.userRepository.findOneBy({id})
